...
import HighschoolProfessorReportsTableHeader from "./HighschoolProfessorReportsTableHeader";

type Props = {
    reportsList: Array<HighschoolProfessorReport>
}

const HighschoolProfessorReportsTable = ({reportsList}: Props) => {

    const [reportsListToDisplay, setReportsListToDisplay] = useState<Array<HighschoolProfessorReport>>(reportsList);
    ...
    const getDownloadPdfFromReportButton = (report: HighschoolProfessorReport) => {

        return (
            <Button className="button mt-0"
                    variant="link"
                    onClick={() => console.log("download hs prof report " + report.id)}>
                <DownloadIcon/>
            </Button>);
    }

    const getDeleteReportButton = (report: HighschoolProfessorReport) => {

        return (
            <Button className="button mt-0"
                    variant="link"
                    onClick={() => console.log("delete hs prof report " + report.id)}>
                <DeleteIcon/>
            </Button>);
    }

    const getEditReportButton = (report: HighschoolProfessorReport) => {

        return (
            <Button className="button mt-0"
                    variant="link"
                    onClick={() => window.open("/highschool-professor-form/" + report.id)}>
                <EditIcon/>
            </Button>);
    }

    const getControls = (report: HighschoolProfessorReport) => {
    ...
    const displayReportDetailsRow = (report: HighschoolProfessorReport) => {
                ...
                <td colSpan={8} role="cell"
                    ...
                    <Container fluid className="mt-1 mb-3 details-box">
                        <Row>
                            ...
                            <Col xs={12} md={4} className="mb-3">
                                <div className="details-label mb-1">Classes
                                </div>
                                <div className="details-text">
                                    {report.classes}
                                </div>
                            ...
                            <Col xs={12} md={4} className="mb-3">
                                <div className="details-label mb-1">Portfolio</div>
                                <div className="details-text">
                                    {report.portfolio}
                                </div>
                            </Col>
                        ...
                        <Row>
                            <Col xs={12} md={4} className="mb-3">
                                <div className="details-label mb-1">Criterion</div>
                                <div className="details-text">
                                    {report.criterionNumber}
                                </div>
                            </Col>
                            <Col xs={12} md={4} className="mb-3">
        ...
        return (
            <div>
                ...
                <table className={"table"}>
                    <HighschoolProfessorReportsTableHeader reportsListToDisplay={reportsListToDisplay}
                                                           setReportsListToDisplay={setReportsListToDisplay}
                                                           updatedReportsListToDisplay={updatedReportsListToDisplay}
                                                           setUpdatedReportsListToDisplay={setUpdatedReportsListToDisplay}/>
                    <tbody>
                            ...
                            return (
                                <React.Fragment key={index}>
                                    <tr key={index} className={"table-row"}>
                                        ...
                                        <td className={"table-body-cell"}>{report.className}</td>
                                        <td className={"table-body-cell"}>{report.teachingProject}</td>
                                        <td className={"table-body-cell"}><strong>{report.sid}</strong></td>
...
export default HighschoolProfessorReportsTable;